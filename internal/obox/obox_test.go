...
func TestObox_DispatchLocalAction(t *testing.T) {
	...
	resPost, _ := m.dispatchLocalAction(m.ctx, ActionPayload{
		URL:     "/test-post",
		Method:  "POST",
		Payload: map[string]string{"test_key": "test_val"},
	})
...
func TestObox_CallOdooOboxConnect(t *testing.T) {
	...
	case <-connectReceived:
		// connectReceived fires inside the HTTP handler, BEFORE callOdooOboxConnect
		// calls SetCredentials. Wait for the goroutine to finish so credentials
		// are guaranteed to be written before we read them.
		select {
		case <-done:
