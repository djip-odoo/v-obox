...
func TestBuildText_EmAndUnderline(t *testing.T) {
	// Em enabled
	gotEm := BuildText("T", TextAttrs{Em: boolRef(true)})
	testutil.ExpectedBytesEqual(t, gotEm, []byte{ESC, 0x45, 0x01, 'T'})

	// Em disabled
	gotEmOff := BuildText("T", TextAttrs{Em: boolRef(false)})
	testutil.ExpectedBytesEqual(t, gotEmOff, []byte{ESC, 0x45, 0x00, 'T'})

	// Underline enabled
	gotUl := BuildText("U", TextAttrs{Underline: boolRef(true)})
	testutil.ExpectedBytesEqual(t, gotUl, []byte{ESC, 0x2D, 0x01, 'U'})

	// Underline disabled
	gotUlOff := BuildText("U", TextAttrs{Underline: boolRef(false)})
	testutil.ExpectedBytesEqual(t, gotUlOff, []byte{ESC, 0x2D, 0x00, 'U'})
}
