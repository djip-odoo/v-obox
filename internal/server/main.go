...
type Server struct {
	...
	obox     *obox.Module
	stopOnce sync.Once
	done     chan struct{} // closed when the Fiber goroutine exits
}
...
func (s *Server) Stop() error {
	...
	s.stopOnce.Do(func() {
		...
		// Fiber owns the listener lifecycle when started via app.Listener();
		// Shutdown() closes the listener and signals the goroutine to exit.
		err = s.app.Shutdown()
		// Wait for the goroutine to exit so the port is fully released
		// before Stop() returns — important for test isolation.
		<-s.done
	})
	return err
}
