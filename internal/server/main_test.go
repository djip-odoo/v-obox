...
// newTestServer provides a centralized, clean test server setup for all server package tests.
func newTestServer(t testing.TB, cfg *config.Manager) (*Server, *printer.Manager) {
	...
	s := New(cfg.Data.Port, mgr, cfg)
	t.Cleanup(func() { _ = s.Stop() })
	return s, mgr
}

// newMockOdoo starts an httptest.Server that mocks Odoo obox JSON-RPC endpoints.
...
func TestCustomSerialNumberSupport(t *testing.T) {
	...
	testutil.ExpectedEqual(t, s.AppID(), "CUSTOM_SERIAL_123")
