...
func TestFindAvailablePort_RangeExhausted(t *testing.T) {
	// Grab a single ephemeral port and hold it. Calling findAvailablePort with
	// start==end==that port guarantees ErrNoAvailablePort without any risk of
	// clashing with the fixed 4545-4555 range used by other packages in parallel.
	ln, err := net.Listen("tcp", "127.0.0.1:0")
	testutil.ExpectedNoError(t, err)
