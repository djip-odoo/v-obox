...
function StatusPill({ type, status }: StatusPillProps) {
  const isPending = type === "cloud" && (status === "connecting" || status === "polling");
  const icon = type === "lan" ? (
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
  ...
  return (
    <div
      className={`inline-flex items-center p-1 rounded-full border ${status === "connected"
        ? "bg-emerald-50 text-emerald-700 border-emerald-200/70"
        : isPending
          ? "bg-amber-50 text-amber-700 border-amber-200/70"
          : "bg-rose-50 text-rose-700 border-rose-200/70"
        }`}
      title={type === "lan" ? `Local Network: ${status}` : `Cloud Status: ${status}`
      }
    >
...
export default function OboxFields() {
  ...
  const appId = status?.appId || appContext.data.app?.appId || "";
  const ipAddress = status?.ipAddress || "";

  if (data.isConnected && status?.dbUrl) {
